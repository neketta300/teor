...
**Классификация** — это задача машинного обучения, где модель должна **отнести объект к одному из заранее известных классов (категорий)**.

Простыми словами: модель смотрит на входные данные и отвечает на вопрос **«Что это?»** или **«К какому типу это относится?»**.

### Ключевая идея

```
Входные данные (признаки)  →  Модель  →  Класс (категория)

Примеры признаков           Ответ модели
─────────────────            ────────────
email с словами "выигрыш"  →  СПАМ (1)
email "привет, как дела?"  →  НЕ СПАМ (0)

клиент неактивен + недоволен  →  УЙДЁТ (1)
клиент активен + доволен      →  ОСТАНЕТСЯ (0)

опухоль большая + неровная    →  ЗЛОКАЧЕСТВЕННАЯ
опухоль маленькая + ровная    →  ДОБРОКАЧЕСТВЕННАЯ
```

### Главное свойство классификации

> **Ответ — всегда ДИСКРЕТНАЯ категория (класс), а не число.**

Модель не говорит «клиент уйдёт на 73%». Она говорит: **«клиент уйдёт»** или **«клиент останется»**. (Хотя внутри модель может считать вероятность, но на выходе — класс.)

### Типы классификации

| Тип | Описание | Пример |
|-----|----------|--------|
| **Бинарная** | 2 класса | Спам / Не спам, Болен / Здоров |
| **Мультиклассовая** | 3+ класса | Кошка / Собака / Птица |
| **Мультилейбловая** | Объект может belong к нескольким классам | Фото: «закат» И «море» И «лодка» |

### Где применяется?

| Область | Задача | Классы |
|---------|--------|--------|
| 📧 Почта | Фильтрация спама | Спам / Не спам |
| 🏦 Банки | Оценка кредитоспособности | Надёжный / Ненадёжный |
| 🏥 Медицина | Диагностика | Болен / Здоров |
| 🔐 Безопасность | Обнаружение мошенничества | Мошенничество / Норма |
| 📱 Телеком | Предсказание оттока | Уйдёт / Останется |
| 🛒 Магазины | Классификация товаров | Электроника / Одежда / Еда |
...
### Как работает модель классификации (на примере логистической регрессии)?

```
1. Модель получает признаки: age=25, balance=1000, is_active=0, satisfaction=2
2. Вычисляет «скор»: z = w1*age + w2*balance + w3*is_active + w4*satisfaction + b
3. Пропускает через сигмоиду: probability = 1 / (1 + e^(-z))
4. Если probability > 0.5 → класс 1 (уйдёт), иначе → класс 0 (останется)
```

Модель **учится** на размеченных данных: ей показывают 1000 клиентов с известным исходом, она подбирает веса `w1, w2, ...` так, чтобы правильно классифицировать как можно больше.
